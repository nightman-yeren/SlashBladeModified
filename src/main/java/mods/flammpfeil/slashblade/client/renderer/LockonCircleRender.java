...
import static mods.flammpfeil.slashblade.util.ExcludedPlayers.isPlayerInList;
...
public class LockonCircleRender {
    ...
    public void onRenderLiving(RenderLivingEvent event){
        ...
        //Hit rule check
        boolean hitPlayer = player.getPersistentData().getBoolean(SlashBlade.modid + "hitPlayer");
        if (!hitPlayer) {
            if (livingEntity instanceof Player) {
                return;
            }
        }
        boolean hitPassive = player.getPersistentData().getBoolean(SlashBlade.modid + "hitPassive");
        if (!hitPassive) {
            if (livingEntity instanceof Animal) {
                return;
            }
        }
        boolean hitAggressive = player.getPersistentData().getBoolean(SlashBlade.modid + "hitAggressive");
        System.out.println(hitAggressive);
        if (!hitAggressive) {
